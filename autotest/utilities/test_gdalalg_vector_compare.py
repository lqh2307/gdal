...
def test_gdalalg_vector_compare_different_field_width_includes(tmp_vsimem):

    gdal.alg.vector.convert(
        input="../ogr/data/poly.shp", output=tmp_vsimem / "out.sqlite"
    )
    with gdal.alg.vector.compare(
        input=tmp_vsimem / "out.sqlite",
        reference="../ogr/data/poly.shp",
        skip_fid=True,
        skip_metadata=True,
    ) as alg:
        assert (
            alg["output-string"]
            == """Layer poly: Field 'area' has width 12 in reference layer and includes sign but width 0 in input layer and does not include sign\nLayer poly: Field 'area' has precision 3 in reference layer and includes decimal separator but precision 0 in input layer and does not include decimal separator\nLayer poly: Field 'eas_id' has width 11 in reference layer, but 0 in input layer\nLayer poly: Field 'prfedea' has width 16 in reference layer, but 0 in input layer\n"""
        )
